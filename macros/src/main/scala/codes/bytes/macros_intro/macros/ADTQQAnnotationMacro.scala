...
object ADT_QQ {
  def impl(c: whitebox.Context)(annottees: c.Expr[Any]*): c.Expr[Any] = {
    ...
    val result: Tree = inputs match {
      case (t @ q"$flags trait $name[..$tparams] extends ..$parents { ..$body }") :: Nil if flags.hasFlag(SEALED) ⇒
      ...
      case (t @ q"$flags trait $name[..$tparams] extends ..$parents { ..$body }") :: Nil ⇒
        ...
        t
      case (cls @ q"$flags class $name[..$tparams] extends ..$parents { ..$body }") :: Nil if flags.hasFlag(ABSTRACT) && flags.hasFlag(SEALED) ⇒ // there's no bitwise AND (just OR) on Flags
      ...
      case (o @ q"$flags object $name") :: Nil ⇒
        ...
        o
      // companions
      case (t @ q"$flags trait $name[..$tparams] extends ..$parents { ..$body }") :: (mD: ModuleDef):: Nil if flags.hasFlag(SEALED) ⇒
        c.info(p, s"ADT Root trait $name sanity checks OK.", force = true)
        q"$t; $mD"
      case (t @ q"$flags trait $name[..$tparams] extends ..$parents { ..$body }") :: (mD: ModuleDef) :: Nil ⇒
        c.error(p, s"ADT Root traits (trait $name) must be sealed.")
        q"$t; $mD"
      case (cls @ q"$flags class $name[..$tparams] extends ..$parents { ..$body }") :: (mD: ModuleDef) :: Nil⇒ // there's no bitwise AND (just OR) on Flags
        c.info(p, s"ADT Root class $name sanity checks OK.", force = true)
        q"$cls; $mD"
      case (cls @ q"$flags class $name[..$tparams] extends ..$parents { ..$body }") :: (mD: ModuleDef) :: Nil ⇒
        c.error(p, s"ADT Root classes (class $name) must be abstract and sealed.")
        q"$cls; $mD"
      // method definition
